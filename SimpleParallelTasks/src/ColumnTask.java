public class ColumnTask implements Runnable {
    ...
    public ColumnTask(Integer start, Integer count, Integer[][] matrix1, Integer[][] matrix2, Integer[][] result) {
        this.start = start;
        this.count = count;
    ...
    @Override
    public void run() {
        int i = start % result.length;  //line of first element to compute
        int j = start / result.length;  //column of first element to compute
        ...
        for (int c = 0; c < count; c++) {
            ...
            i++;
            if (i == result.length) {   //reset row if equal to number of rows
                i = 0;
                j++;
            }
        }
